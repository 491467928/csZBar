...
public class ZBarScannerActivity extends Activity
	...
	private void setUpCamera() {
		...
		drawSight = params.optBoolean("drawSight", true);
		...
		for (ZBarcodeFormat format : getFormats()) {
			//解决误扫问题
			if (format.getId() == ZBarcodeFormat.DATABAR.getId()) {
...
//		}
		...
		if(drawSight){
			ScanFinderSurfaceView finderView = new ScanFinderSurfaceView(this);
			finderView.setLayoutParams(new FrameLayout.LayoutParams(
					ViewGroup.LayoutParams.MATCH_PARENT,
					ViewGroup.LayoutParams.MATCH_PARENT,
					Gravity.CENTER
			));
			...
			finderWidth = (metric.widthPixels * 5 / 8);
			finderHeight = finderWidth;
			finderLeft=(metric.widthPixels-finderWidth)/2;
			finderTop=(metric.heightPixels-finderHeight)/2;
			finderView.setFinderRect(finderWidth,finderHeight);
			scannerView.addView(finderView);
		}
	...
	private PreviewCallback previewCb = new PreviewCallback() {
		public void onPreviewFrame(byte[] data, Camera camera) {
			...
			if(drawSight){
				//进行裁剪，避免误扫
				// x,y,width,height 图片是旋转了90度的
				barcode.setCrop(finderTop,finderLeft,finderHeight,finderWidth);
			}
